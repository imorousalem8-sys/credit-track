...
export default function AchatsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="card" style={{ background: '#fff', borderRadius: '14px', border: '1px solid #E2E8F0', padding: expenses.length === 0 ? '16px' : 0, overflow: 'hidden' }}>
        {expenses.length === 0 ? (
          <div className="empty-state-box">
            <div className="empty-state-icon-box">
              <Receipt size={26} />
            </div>
            <div className="empty-state-title">Aucun achat ou charge enregistré</div>
            <div className="empty-state-desc">
              Saisissez vos factures fournisseurs, achats de marchandises et frais généraux pour calculer la TVA déductible.
            </div>
            <Link 
              href="/comptabilite"
              className="empty-state-cta"
            >
              <PlusCircle size={16} /> Saisir une première charge
            </Link>
          </div>
        ) : (
