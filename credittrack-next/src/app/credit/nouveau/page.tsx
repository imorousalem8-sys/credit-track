...
export default function NewCreditSalePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Supabase Client insert/update
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Enregistrez une nouvelle vente à crédit</p>
        </div>

        <Link 
          href="/credit" 
          className="btn btn-outline bg-white border-slate-200 text-slate-700 hover:text-slate-900 flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div>
            <div className="text-xs font-bold text-slate-500">Montant Total</div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div>
            <div className="text-xs font-bold text-slate-500">Date Limite</div>
        ...
        {/* Card 4: Mode de Paiement */}
        <div className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-3.5 shadow-sm">
          <div className="w-11 h-11 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
            <CreditCard size={22} />
          </div>
          <div>
            <div className="text-xs font-bold text-slate-500">Mode de Paiement</div>
            <div className="text-base font-extrabold text-slate-900 mt-0.5">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <select 
              ...
              className="form-control flex-1 h-11 rounded-xl text-sm font-medium border-slate-200"
            >
            ...
            <button 
              ...
              className="btn btn-outline text-blue-600 border-blue-200 bg-blue-50/50 hover:bg-blue-100/50 font-bold text-xs h-11 px-5 rounded-xl flex items-center justify-center gap-1.5 transition shrink-0"
            >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-center">
                        <input 
                          ...
                          className="form-control h-10 text-sm text-center rounded-lg border-slate-200 max-w-[90px] mx-auto"
                        />
                      </td>
                      <td className="py-2.5 px-3 text-right">
                        <input 
                          ...
                          placeholder="0"
                          className="form-control h-10 text-sm text-right rounded-lg border-slate-200 max-w-[140px] ml-auto"
                        />
                      </td>
                      <td className="py-2.5 px-3 text-right font-extrabold text-blue-600 text-sm">
          ...
          <button 
            type="button"
            onClick={addRow}
            className="w-full py-2.5 border-2 border-dashed border-blue-200 bg-blue-50/30 hover:bg-blue-50/70 text-blue-600 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition"
          >
          ...
          <div className="flex justify-end items-center gap-4 pt-3 border-t border-slate-100">
            ...
            <div className="bg-blue-50 text-blue-600 text-lg font-black px-6 py-2.5 rounded-xl border border-blue-200">
              {formatAfricanCurrency(totalGrossAmount, currency)}
            </div>
          </div>
        </div>

        {/* Row 4: 2 Columns (Payment Details 50% | Warranty 50%) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          
          {/* Left: Détails de paiement */}
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
            <div className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
              <CreditCard size={18} className="text-blue-600" /> Détails de paiement
            </div>
            ...
            <div>
              ...
              <select 
                ...
                className="form-control h-11 text-sm rounded-lg border-slate-200"
              >
            ...
            <div>
              ...
              <input 
                ...
                className="form-control h-11 text-sm rounded-lg border-slate-200 font-bold text-blue-600"
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
            <div className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
              <ShieldCheck size={18} className="text-blue-600" /> Garantie <span className="text-xs text-slate-400 font-medium">(optionnel)</span>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-2">
          <div className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
            <FileText size={18} className="text-blue-600" /> Notes ou informations complémentaires <span className="text-xs text-slate-400 font-medium">(optionnel)</span>
          </div>
          <input 
            type="text"
            value={notes}
        ...
        <div className="flex justify-end items-center gap-3 pt-2">
          <Link 
            href="/credit"
            className="btn btn-outline bg-white border-slate-300 text-slate-600 hover:text-slate-900 px-6 py-2.5 rounded-xl text-sm font-bold shadow-sm"
          >
            Annuler
          </Link>
          <button 
            type="submit"
            disabled={loading}
            className="btn bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded-xl text-sm font-extrabold flex items-center gap-2 shadow-lg shadow-blue-600/30 transition disabled:opacity-50"
          >
